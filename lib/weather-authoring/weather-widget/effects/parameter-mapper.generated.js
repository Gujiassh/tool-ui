...
export function getMoonPhase(timestamp) {
    ...
    const phaseDays = ((daysSinceNewMoon % synodicMonth) + synodicMonth) % synodicMonth;
...
export function getSceneBrightnessFromTimeOfDay(timeOfDay, conditionCode = "clear") {
...
export function mapWeatherToEffects(params) {
    const { conditionCode, windSpeed, precipitationLevel, visibility, timestamp, timeOfDay: explicitTimeOfDay, } = params;
    ...
    const atmosphere = {
        ...
        starVisibility: isNight && !preset.cloud
            ? 1.0
            : isNight
    ...
    const bloomConditionBoost = conditionCode === "fog"
        ? 0.18
