...
import { releaseWeatherWebglBudgetSlotOnInitFailure, releaseWeatherWebglCanvasBudgetSlot, tryAcquireWeatherWebglCanvasBudgetSlot, } from "./weather-webgl-budget";
import { CELESTIAL_FRAGMENT, CLOUD_FRAGMENT, COMPOSITE_FRAGMENT, FULLSCREEN_VERTEX, LIGHTNING_FRAGMENT, RAIN_FRAGMENT, SNOW_FRAGMENT, } from "./generated/weather-effect-shaders.generated";
import { createFramebuffer, createProgram, resizeFramebuffer, } from "./weather-effect-gl";
import { clearOffscreenPass, isLightningPassActive, renderCelestialPass, renderCloudPass, renderCompositePass, renderLightningPass, renderRainPass, renderSnowPass, } from "./weather-effect-render-passes.generated.js";
export function useWeatherEffectsRenderer(props) {
    ...
    const failInit = useCallback(({ canvas, contextLost = false, markInitFailed = true, warnMessage, errorMessage, }) => {
        if (contextLost) {
        ...
        disposeGL();
        hasWebglBudgetSlotRef.current = releaseWeatherWebglBudgetSlotOnInitFailure(canvas, hasWebglBudgetSlotRef.current);
        return false;
    }, [disposeGL]);
    const initGL = useCallback(() => {
        ...
        if (!canvas)
            return false;
        if (hasWebglBudgetSlotRef.current === false)
            return false;
        if (hasWebglBudgetSlotRef.current === null) {
            ...
            if (!ok) {
                ...
                if (process.env.NODE_ENV !== "production") {
                    console.warn("[WeatherEffectsCanvas] Too many WebGL canvases on the page; rendering this widget without effects.");
                }
                return false;
            }
        ...
        if (moonTexture) {
            ...
            image.onload = () => {
                ...
                if (!glCurrent || moonTextureRef.current !== moonTexture)
                    return;
    ...
    const render = useCallback(() => {
        ...
        if (runtimeProps.layers.lightning &&
        ...
        const lightningActive = isLightningPassActive(runtimeProps.layers, runtimeProps.lightning, programs.lightning, time, lastFlashTimeRef.current);
