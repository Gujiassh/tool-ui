...
export const toolkit: Toolkit = {
  ...
  render: ({ args, result, status, addResult, toolCallId }) => {
    ...
    >({
      ...
      result,
      safeParseArgs: safeParseSerializableMessageDraft,
      loadingMessage: "Loading draft…",
      awaitingActionMessage: "Review draft",
    });
    ...
    return (
      <MessageDraftErrorBoundary>
        {state.kind === "partial" || state.kind === "ready" ? (
          <MessageDraft {...state.args} outcome={state.result} />
        ) : (
          <MessageDraft
            {...state.args}
            onSend={async () => {
              // Perform actual send logic here
              await sendMessage(state.args);
              addResult("sent");
            }}
