...
import { resolveStreamingToolRenderState } from "@/components/tool-ui/shared";
  ...
  showXPost: {
    ...
    render: ({ result, status }) => {
      const state = resolveStreamingToolRenderState({
        status,
        result,
        safeParseResult: safeParseSerializableXPost,
...
The same pattern works for `InstagramPost` and `LinkedInPost` by swapping the component and safe parser (`safeParseSerializableInstagramPost` / `safeParseSerializableLinkedInPost`).
