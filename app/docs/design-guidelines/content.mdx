...
- **Information:** Display data. Users read more than they click. Examples: [Data Table](/docs/data-table), [Chart](/docs/chart), [Link Preview](/docs/link-preview), [Citation](/docs/citation).

- **Decision:** Capture choices that matter (approve/reject, send/cancel). These need clear options and [receipts](/docs/receipts) that prove what happened. Examples: [Approval Card](/docs/approval-card), [Option List](/docs/option-list), [Order Summary](/docs/order-summary).
...
- **Redundant narration:** The assistant shouldn't describe what the tool UI already shows. The tool UI shouldn't echo what the assistant just said. If the assistant asks "Where to?", the tool UI doesn't need a header saying "Where to?"; show the options. Divide the work: the assistant provides context, the tool UI provides structure.
