...
CodeBlock renders agent-produced code with syntax highlighting, line numbers, and a copy button. You can highlight specific lines to draw attention to changes or bugs, and optionally collapse long snippets by setting `maxCollapsedLines` so conversations stay scannable. Without it, code appears as unstyled monospace text.
...
export const toolkit: Toolkit = {
  showCodeBlock: {
    ...
    render: createResultToolRenderer({
      ...
      render: (parsedResult) => (
        <CodeBlock.Standard {...parsedResult} />
      ),
    }),
  },
};
