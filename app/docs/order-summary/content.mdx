...
export const toolkit: Toolkit = {
  ...
  render: ({ args, result, status, addResult, toolCallId }) => {
    ...
    >({
      ...
      args: {
        ...args,
        id: (args as any)?.id ?? `order-${toolCallId}`,
      },
      result,
      safeParseArgs: safeParseSerializableOrderSummary,
      loadingMessage: "Loading order…",
      awaitingActionMessage: "Review order",
    });
    ...
    return (
      <OrderSummaryErrorBoundary>
        {state.kind === "partial" || state.kind === "ready" ? (
          <OrderSummary {...state.args} choice={state.result} />
        ) : (
          <OrderSummary
            {...state.args}
            onResponseAction={(actionId) => {
