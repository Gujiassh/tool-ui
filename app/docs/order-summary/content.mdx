...
Shopping experiences need a clear receipt before the user commits. OrderSummary renders line items with images, quantities, and an itemized price breakdown. Use `OrderSummary.Display` during confirmation and `OrderSummary.Receipt` after a confirmed decision.
