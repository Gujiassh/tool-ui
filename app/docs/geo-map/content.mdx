...
<Step title="Import Leaflet styles">

Leaflet requires global CSS in your app entry.

```tsx
import "leaflet/dist/leaflet.css";
```

If you are using the Tool UI agent skill, ask it to add the Leaflet CSS import to your root layout automatically.
...
Registration tells assistant-ui which component to render when a tool returns geo payloads. Without it, tool results appear as raw JSON.
...
GeoMap shell styling is scoped to `[data-slot="geo-map"]` and uses CSS custom properties. This keeps package defaults aligned with shadcn tokens while letting you override just the pieces you need.
