...
<FeatureGrid>
  ...
  <Feature icon="CheckCircle2" title="Receipt state">
    Chosen selections render as read-only receipts in conversation history
  </Feature>
</FeatureGrid>

## Receipt State

When `choice` is set, the component renders in a read-only receipt state showing only the chosen selection(s). Use this to display past decisions in conversation history.

The receipt state:

- Shows only the chosen option(s) with a checkmark
- Hides unselected options and selection actions
...
<TypeTable
  type={{
    ...
    selectionActions: {
      description: "Selection action buttons",
