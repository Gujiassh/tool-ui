...
export const toolkit: Toolkit = {
  ...
  render: ({ args, status, toolCallId }) => {
    ...
    >({
      ...
      args: {
        ...args,
        version: (args as any)?.version ?? "3.1",
        id: (args as any)?.id ?? `weather-${toolCallId}`,
      },
      safeParseArgs: safeParseWeatherWidgetPayload,
...
<TypeTable
  type={{
    ...
    visual: {
      description: "Deterministic visual time source",
