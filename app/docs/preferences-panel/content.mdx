...
export const toolkit: Toolkit = {
  ...
  render: ({ args, status, addResult, toolCallId }) => {
    const state = resolveStreamingToolRenderState<
      SerializablePreferencesPanel,
      { status: "saved" | "cancelled"; values?: Record<string, unknown> }
    >({
      status,
      args: {
        ...args,
      ...
      safeParseArgs: safeParseSerializablePreferencesPanel,
      loadingMessage: "Loading preferences…",
      awaitingActionMessage: "Adjust preferences",
    });
