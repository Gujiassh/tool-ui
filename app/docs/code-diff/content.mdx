...
Dumping raw before-and-after text into a chat message forces users to play spot-the-difference. CodeDiff renders syntax-highlighted diffs with additions and deletions marked clearly, so changes are obvious at a glance. Pass `oldCode` and `newCode` to compare two versions of a file, or pass a `patch` string for unified diffs straight from version control.
