...
<DocsHeader
  title="Actions"
  description="How users interact with tool results — local actions for in-context controls, decision actions for choices that continue the conversation."
...
Tool UI provides two action surfaces:
...
- **`ToolUI.DecisionActions`** — Consequential choices that produce a durable decision envelope. Approving, confirming, selecting. The result flows back to the assistant and the component shows a [receipt](/docs/receipts).
