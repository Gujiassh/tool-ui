...
const toolkit: Toolkit = {
  previewLink: {
    ...
    render: ({ result, status }) => {
      ...
      return (
        <LinkPreviewErrorBoundary>
          <LinkPreview {...state.result} maxWidth="420px" />
        </LinkPreviewErrorBoundary>
      );
    },
  },
...
export const toolkit: Toolkit = {
  ...
  render: ({ args, result, status, addResult, toolCallId }) => {
    ...
    const optionList = state.args;

    if (state.kind === "partial" || state.kind === "ready") {
      return (
        <OptionList {...optionList} value={undefined} choice={state.result} />
      );
    }
